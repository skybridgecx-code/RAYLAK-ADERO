...
import { assignComplianceIssue, type AssignmentActionState } from "./actions";
...
export function AssignComplianceForm({
  memberType,
  profileId,
  documentType,
  currentAssignee,
  ...
  const [state, formAction, isPending] = useActionState(assignComplianceIssue, initialState);
    ...
    <form action={formAction} className="space-y-2">
      ...
      <div className="flex flex-wrap items-end gap-2">
        <div className="flex-1 min-w-32">
          ...
          <input
            name="assignedTo"
            placeholder={currentAssignee ?? "Owner name"}
          ...
          />
          <FieldError messages={state.fieldErrors["assignedTo"]} />
        </div>
        ...
        <button
          ...
          style={{ background: "#1e293b", color: "#94a3b8", border: "1px solid rgba(255,255,255,0.12)" }}
        >
      ...
      {state.saved && (
        <p className="text-xs" style={{ color: "#4ade80" }}>
          {currentAssignee ? "Reassigned." : "Assigned."}
        </p>
      )}
    </form>
  );
